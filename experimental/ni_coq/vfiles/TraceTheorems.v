...
Proof.
    ...
    - destruct t'.
        ...
        assert (E1: length t'0 = 1). {
            assert (E': length t'0 = length (p0 :: t')) by
                (apply (state_upd_node_preserves_len t'0 (p0 :: t')
                (state_upd_node id (RecordSet.set ncall (fun _ : call => c') n) s')
                ); assumption).
...
Proof.
    ...
    - assert (E: t = a0::t'). {
        inversion H; subst.
        assert (E1: exists a', t'0 = a' :: a0 :: t'). {
            apply (trace_upd_head_preserves_tail t'0 (a0 :: t') a
            (state_upd_node id (RecordSet.set ncall (fun _ : call => c') n) s')).
            assumption.
        }
