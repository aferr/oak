...
Inductive step_node_ev (id: node_id): call -> state -> down_l ->
    ...
    | SWriteChanEv s nlbl han msg s':
        ...
        step_node_ev id (WriteChannel han msg) s (nlbl DWNnil)
            s' (nlbl ---> msg)
    ...
    | SWriteChanDwnEv s nlbl han msg s' ell':
        ...
        step_node_ev id (WriteChannelDown han msg ell') s (msg DWN nlbl |--> ell')
            s' (nlbl ---> msg)
    | SReadChanEv s nlbl han chan msg s':
        (s.(nodes) .[?id]).(lbl) = nlbl ->
        step_node id (ReadChannel han) s s' ->
        msg_is_head chan msg ->
        step_node_ev id (ReadChannel han) s (nlbl DWNnil)
            s' (nlbl <--- msg)
    | SCreateChanEv s nlbl clbl s':
        ...
        * contents of channels indirectly via reads *)
        ...
        step_node_ev id (CreateChannel clbl) s (nlbl DWNnil)
            s' (nlbl --- )
    | SCreateNodeEv s nlbl new_lbl h s':
        (s.(nodes) .[?id]).(lbl) = nlbl ->
        step_node id (CreateNode new_lbl h) s s' ->
        step_node_ev id (CreateNode new_lbl h) s (nlbl DWNnil)
            s' ( -- nlbl -- )
    | SWaitOnChannelsEv s hs nlbl s':
        (s.(nodes) .[?id]).(lbl) = nlbl ->
        step_node id (WaitOnChannels hs) s s' ->
        step_node_ev id (WaitOnChannels hs) s (nlbl DWNnil)
            s' (nlbl ---)
    | SChannelCloseEv s han nlbl s':
        (s.(nodes) .[?id]).(lbl) = nlbl ->
        step_node id (ChannelClose han) s s' ->
        step_node_ev id (ChannelClose han) s (nlbl DWNnil)
            s' (nlbl ---)
    | SNodeLabelReadEv s nlbl s':
        (s.(nodes) .[?id]).(lbl) = nlbl ->
        step_node id NodeLabelRead s s' ->
        step_node_ev id NodeLabelRead s (nlbl DWNnil)
            s' (nlbl <--L nlbl)
    | SChannelLabelReadEv s han nlbl clbl s':
        (s.(nodes) .[?id]).(lbl) = nlbl ->
        (s.(chans) .[?han]).(lbl) = clbl ->
        step_node id (ChannelLabelRead han) s s' ->
        step_node_ev id (ChannelLabelRead han) s (nlbl DWNnil)
            s' (nlbl <--L clbl)
    | SInternalEv s nlbl s':
        (s.(nodes) .[?id]).(lbl) = nlbl ->
        step_node id Internal s s' ->
        step_node_ev id Internal s (nlbl DWNnil)
            s' (nlbl ---).

Inductive step_system_ev: state -> down_l -> state -> event_l -> Prop :=
    | SytsemEvSkip s ell: step_system_ev s (ell DWNnil) s (ell ---) 
    | SystemEvStepNode id n c c' s d s' e:
        (s.(nodes).[?id]).(obj) = Some n ->
        n.(ncall) = c ->
        step_node_ev id c s d s' e ->
        ...
        step_system_ev s d s'' e.

Definition trace := list (state * down_l * event_l).
